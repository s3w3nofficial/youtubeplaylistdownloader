...
default_playlist = "https://www.youtube.com/playlist?list=PLTAeE2fqeJQJrRYiNKfjNR-GWBH7TZJNZ"

def get_playlist_links(playlist_url):
    page_elements = urlopen(playlist_url).readlines()
    video_elements = [el for el in page_elements if 'pl-video-title-link' in str(el)]  # Filter out unnecessary lines
    video_urls = [str(v).split('href="',1)[1].split('" ',1)[0] for v in video_elements]  # Grab the video urls from the elements
...
parser.add_argument('-p', '--playlisturl', help='url of the playlist to be downloaded', default=default_playlist, metavar='')
...
confirmation = input('You are about to download {} videos to {}\nWould you like to continue? [Y/n] '.format(len(video_urls), os.path.abspath(args.destination)))

if confirmation.lower() in ['y', '']:
    for u in video_urls:
        yt = pytube.YouTube(u)
        vid = yt.streams.filter(only_audio=True, subtype='mp4').all()[-1] # grab the highest resolution mp4 file
        if vid.default_filename in str(directory_contents):
